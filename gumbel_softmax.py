...
class DiffPool(torch.nn.Module):
    def __init__(self, dataset, num_layers=5, hidden=64, hop=2, num_patches=5, ratio=0.25, plot=False, dropout=False, ge=False, gs=False, total=20, hard=False, hard_train=False, aux_loss=False, decay_rate=1):
    ...
    def forward(self, data, temp):
       ...
       # !!! max / avg of graph embeddings
#         x = x.max(dim=1)[0]
#         if self.dropout:
#             x = F.dropout(x, p=0.2, training=self.training)
#         x = self.lin1(x)

       # !!! summation aggreator after linear layer
